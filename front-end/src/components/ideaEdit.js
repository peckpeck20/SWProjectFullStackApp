...
class IdeaEdit extends Component {
  ...
  onSubmit(e) {
    ...
    if (
      editIdea.title !== "" &&
      editIdea.description !== "" &&
      editIdea.budget !== "" &&
      editIdea.peopleNeeded !== "" &&
      editIdea.categoryId !== ""
    ) {
  ...
  render() {
    return (
      <div className="container">
        <h3 style={customStyle.heading}>
          EDIT IDEA {this.props.chosenIdea.id}
        </h3>
        <form onSubmit={this.onSubmit.bind(this)}>
          <p>Title</p>
          <input
            name="title"
            value={this.state.editIdea.title}
            onChange={this.inputChanged}
          />
          <p>Description</p>
          <input
            name="description"
            value={this.state.editIdea.description}
            onChange={this.inputChanged}
          />
          <p>Budget</p>
          <input
            type="number"
            name="budget"
            value={this.state.editIdea.budget}
            onChange={this.inputChanged}
          />
          <p>
            Ready for comment?{" "}
          ...
          <input
            type="number"
            name="peopleNeeded"
            value={this.state.editIdea.peopleNeeded}
            onChange={this.inputChanged}
          />
          <p>Category id</p>
          <input
            type="number"
            name="categoryId"
            value={this.state.editIdea.categoryId}
            onChange={this.inputChanged}
          />{" "}
          <br />
          <Link to="/">
          ...
          <Link to={`/ideas/${this.props.chosenIdea.id}`}>
            <button
              className="button button-delete button-text"
              style={customStyle.button}>
              CANCEL
            </button>
          </Link>
  ...
  inputChanged = (event) => {
    ...
    this.setState({
      editIdea: Object.assign({}, this.state.editIdea, {
