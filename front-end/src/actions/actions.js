...
export function ideaCreate(value, dispatch) {
  return axios.post(`${ROOT_URL}/idea`, value).then((response) => dispatch(receiveData(IDEA_CREATE, response.data)));
}

export function ideaDelete(id, dispatch) {
  return axios.delete(`${ROOT_URL}/idea/${id}`).then((response) => dispatch(receiveData(IDEA_DELETE, response.data)));
}
